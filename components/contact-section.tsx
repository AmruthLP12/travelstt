    ...
    <section className="mb-16" data-aos="fade-up" data-aos-delay="400">
      <Card className="overflow-hidden border-0 border-gray-800 bg-gray-900 shadow-2xl rounded-3xl">
        <div className="bg-gradient-to-r from-indigo-800 to-purple-900 py-12 px-8 text-center">
          <h2 className="text-3xl md:text-4xl font-bold text-white">Contact us today to book your ride!</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {contactInfo.map((info) => {
              ...
              return (
                ...
                >
                  <div
                    className={`w-20 h-20 rounded-full bg-gradient-to-r ${info.type === "email" ? "from-amber-600 to-orange-600" : "from-indigo-600 to-purple-700"} flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300`}
                  >
                    <IconComponent className="h-10 w-10 text-white" />
                  </div>
                  <h3 className="font-bold text-xl text-white mb-2">
                    {info.type === "phone" ? "Call Us" : info.type === "email" ? "Email Us" : "Visit Us"}
                  </h3>
                  <p className="text-indigo-400 font-semibold text-lg">{info.value}</p>
                  <p className="text-gray-400 mt-2 text-center">{info.description}</p>
                </div>
              )
            })}
          </div>
